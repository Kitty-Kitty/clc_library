...
struct cp_ipc_info_s
{
	...
	cp_device_info_t				*cener_server[CP_IPC_MAX_CENTER_SERVER_NUMBER];							//表示中心设备列表
	cp_device_info_t				*client_background_server[CP_IPC_MAX_CLIENT_SERVER_NUMBER];				//表示回显设备列表
	cp_device_info_t				*ipc_background_server[CP_IPC_MAX_IPC_SERVER_NUMBER];				//表示背景设备列表
	cp_device_info_t				*media_server[CP_IPC_MAX_MEDIA_SERVER_NUMBER];							//表示媒体设备列表
