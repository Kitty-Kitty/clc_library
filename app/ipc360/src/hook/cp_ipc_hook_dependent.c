...
**************************************************************************/
#include "cp_log.h"
#include "cp_ipc_hook_dependent.h"
...
cp_int32_t cp_css_add_entry_dependent(cp_ipc_hook_manager_t *manager, cp_css_rtp_hook_type_e type, ...)
{
	...
	cp_ipc_hook_t							*tmp_hook = CP_NULL;
	cp_char_t								tmp_buf[CP_APP_DEFAULT_BUFFER_LENGTH] = { 0, };
	va_list									tmp_args;


	tmp_hook = (cp_ipc_hook_t*)manager->hook_array[CP_GET_LIBRARY_SUB_MODULE_TYPE(type)];
	if (!tmp_hook) {
	...
	ret_int = cp_ipc_hook_dependent(manager, tmp_hook, &tmp_hook->entry, &tmp_args);
	ret_int = cp_ipc_hook_dependent(manager, tmp_hook, &tmp_hook->right, &tmp_args);
	if (ret_int) {
...
cp_int32_t cp_css_add_error_dependent(cp_ipc_hook_manager_t *manager, cp_css_rtp_hook_type_e type, ...)
{
	...
	va_start(tmp_args, type);
	ret_int = cp_ipc_hook_dependent(manager, tmp_hook, &tmp_hook->error, &tmp_args);
	if (ret_int) {
...
cp_int32_t cp_ipc_hook_dependent(cp_ipc_hook_manager_t *manager, cp_ipc_hook_t *hook, cp_queue_t *queue, va_list *args)
{
	...
	cp_ipc_hook_t							*tmp_hook = CP_NULL;
	cp_ipc_hook_block_t						*tmp_hook_block;
	...
	while (1)
	{
		...
		tmp_hook_block = (cp_ipc_hook_block_t*)cp_palloc(manager->pool, cp_sizeof(cp_ipc_hook_block_t));
		if (!tmp_hook_block) {
			cp_log_warn(manager->logger, "%s hook dependent type[%d] error. create [cp_ipc_hook_block_t] error.",
				print_module_register_info(&hook->register_info, tmp_buf, CP_APP_DEFAULT_BUFFER_LENGTH),
				tmp_type);
			return -3;
		}
		tmp_hook_block->type = tmp_type;
		tmp_hook_block->hook = (cp_ipc_hook_t *)manager->hook_array[CP_GET_LIBRARY_SUB_MODULE_TYPE(tmp_type)];
