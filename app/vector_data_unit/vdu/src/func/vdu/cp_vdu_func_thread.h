...
#define		CP_VDU_FUNC_DEFAULT_THREAD_NUMBER						3					//表示定义的最大线程数量
#define		CP_VDU_FUNC_DEFAULT_INTERVAL_MS							10LL				//表示定义线程默认的休眠时间
#define		CP_VDU_FUNC_DEFAULT_QUEUE_SIZE							1024				//表示定义默认的队列大小
...
struct cp_vdu_func_thread_s
{
	...
	cp_vdu_func_thread_status_e			status;							//表示线程运进状态
	cp_vdu_func_thread_manager_t		*dftm;							//表示处理线程管理器
	cp_thread_t							*thread;						//表示当前线程
	cp_int64_t							interval_ms;					//表示休眠时间，单位微秒（ms）
	cp_vdu_func_thread_cb				pft_cb;							//表示处理线程的处理
};
