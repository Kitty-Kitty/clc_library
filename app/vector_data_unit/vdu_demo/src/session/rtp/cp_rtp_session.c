...
/*销毁模块*/
cp_int32_t destroy_rtp_session(cp_vdu_demo_session_manager_t *manager, cp_rtp_session_t* rtp_session)
{
...
cp_int32_t init_rtp_session(cp_rtp_session_t *rtp_session, const cp_rtp_session_info_t *rtp_session_info)
{
	...
	cp_vdu_demo_hook_manager_t					*tmp_manager;
	...
	tmp_manager = (cp_vdu_demo_hook_manager_t*)cp_get_manager(CP_NULL, &tmp_module_manager_info);
	if (!tmp_manager) {
	...
	tmp_hook = (cp_vdu_demo_hook_t*)cp_manager_get_module(tmp_manager, &tmp_module_info);
	if (!tmp_hook) {
