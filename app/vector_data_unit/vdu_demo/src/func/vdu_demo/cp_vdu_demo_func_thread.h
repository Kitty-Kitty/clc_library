...
#ifndef _CP_VDU_DEMO_FUNC_THREAD_INCLUDE_H_
#define _CP_VDU_DEMO_FUNC_THREAD_INCLUDE_H_



#include "cp_lockfree_queue.h"
#include "cp_vdu_demo_func_manager_core.h"




#define		CP_VDU_DEMO_FUNC_DEFAULT_THREAD_NUMBER						3					//表示定义的最大线程数量
#define		CP_VDU_DEMO_FUNC_DEFAULT_INTERVAL_MS							10LL				//表示定义线程默认的休眠时间
#define		CP_VDU_DEMO_FUNC_DEFAULT_QUEUE_SIZE							1024				//表示定义默认的队列大小
...
typedef struct cp_vdu_demo_func_chunk_s									cp_vdu_demo_func_chunk_t;
typedef enum cp_vdu_demo_func_thread_status_enum							cp_vdu_demo_func_thread_status_e;
typedef void(*cp_vdu_demo_func_thread_cb)(cp_vdu_demo_func_thread_t* dft);



#pragma pack(1)

enum cp_vdu_demo_func_thread_status_enum
{
	cp_vdu_demo_func_thread_status_init = 1,
	cp_vdu_demo_func_thread_status_start,
	cp_vdu_demo_func_thread_status_stop,
};

/************************************************************************/
/* 定义func线程管理器                                                    */
/************************************************************************/
struct cp_vdu_demo_func_thread_manager_s
{
...
/* 定义func线程                                                         */
/************************************************************************/
struct cp_vdu_demo_func_thread_s
{
	cp_int32_t							index;							//表示处理线程索引号
	cp_vdu_demo_func_thread_status_e		status;							//表示线程运进状态
	cp_vdu_demo_func_thread_manager_t		*dftm;							//表示处理线程管理器
	cp_thread_t							*thread;						//表示当前线程
	cp_int64_t							interval_ms;					//表示休眠时间，单位微秒（ms）
	cp_vdu_demo_func_thread_cb			pft_cb;							//表示处理线程的处理
};
...
/* 定义处理内容结构                                                      */
/************************************************************************/
struct cp_vdu_demo_func_chunk_s
{
