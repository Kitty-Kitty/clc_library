...
cp_int32_t create_media_play(cp_vdu_demo_t *vdu_demo, cp_play_channel_info_t *cp_info)
{
	...
	for (i = 0; i < (cp_int32_t)cp_array_max_size(&cp_info->mci_array); i++) {
		...
		//不在同一个组，则不做处理
		if (tmp_mci->group_id != tmp_vdu_demo_info->device_info->host.group) {
			continue;
		}
		...
		{
			tmp_module = create_audio_media_play(vdu_demo, tmp_mci);
		}
		break;
		 case cp_media_process_type_video:
		{
			tmp_module = create_video_media_play(vdu_demo, tmp_mci);
		}
		break;
		default:
		{
			cp_app_log(error, vdu_demo, "create_media_play() error. unknow media type[%d]", tmp_mci->media_info.type);
		}
		break;
		}
	}
...
{
	return stop_media_play(vdu_demo, cp_info);
}
...
cp_int32_t stop_media_play(cp_vdu_demo_t *vdu_demo, cp_play_channel_info_t *cp_info)
{
	...
	for (i = 0; i < (cp_int32_t)cp_array_max_size(&cp_info->mci_array); i++) {
		...
		//根据不同的媒体类型创建各种处理过程
		ret_int = stop_av_media_play_module(vdu_demo, tmp_mci);
		if (ret_int) {
			cp_app_log(error, vdu_demo, "stop_av_media_play_module() error. unknow media type[%d]", tmp_mci->media_info.type);
			continue;
		}
...
{
	return create_av_media_play_module(vdu_demo, mci, cp_vdu_demo_manager_type_audio_func, cp_vdu_demo_module_type_audio_func);
}

//创建一个视频处理模块
cp_module_t* create_video_media_play(cp_vdu_demo_t *vdu_demo, cp_media_channel_info_t *mci)
{
	return create_av_media_play_module(vdu_demo, mci, cp_vdu_demo_manager_type_video_func, cp_vdu_demo_module_type_video_func);
}

//创建需要的音视频模块
cp_module_t* create_av_media_play_module(cp_vdu_demo_t *vdu_demo, cp_media_channel_info_t *mci,
	cp_int32_t manager_type, cp_int32_t module_type)
{
	...
	//获取音频处理管理器
	tmp_manager = cp_get_manager(vdu_demo, &tmp_manager_info);
	if (!tmp_manager) {
		cp_app_log(error, vdu_demo, "create_av_media_play() error. not found manager_type[%d] error.",
			manager_type);
		return CP_NULL;
	}
	...
	if (!tmp_module) {
		...
		if (!tmp_module) {
			cp_app_log(error, vdu_demo, "create_av_media_play() error. create module_type[%d] error.",
				module_type);
			return CP_NULL;
		}
	}
		...
		//如果存在则正常处理
		cp_app_log(warn, vdu_demo, "create_av_media_play() warning. create module_type[%d] existed.",
			module_type);
		//return CP_NULL;
	}
	...
	if (ret_int) {
		...
		cp_app_log(error, vdu_demo, "create_av_media_play() error. init %s error.",
	...
	if (ret_int) {
		//初始化失败，则销毁模块内容
		cp_manager_destroy_module(tmp_manager, tmp_module);
		cp_app_log(error, vdu_demo, "create_av_media_play() error. open %s error.",
			print_module_register_info(&tmp_module->register_info, tmp_buf, CP_APP_DEFAULT_BUFFER_LENGTH));
		return CP_NULL;
	}
	...
	insert_channel(&tmp_vdu_demo_info->channel_manager, &tmp_channel);
...
cp_int32_t stop_av_media_play_module(cp_vdu_demo_t *vdu_demo, cp_media_channel_info_t *mci)
{
	...
	//先查找一个模块是否存在
	tmp_ret_channel = find_channel(&tmp_vdu_demo_info->channel_manager, mci->channel_info.index);
	if (!tmp_ret_channel) {
		cp_app_log(error, vdu_demo, "find_channel() warning. not found module index[%d].",
			mci->channel_info.index);
		return -2;
	}
	tmp_channel = *tmp_ret_channel;
	remove_channel(&tmp_vdu_demo_info->channel_manager, mci->channel_info.index);

	//通过set_option_module接口关闭播放
	ret_int = cp_set_option_module(tmp_channel.module,
		cp_vdu_demo_func_set_option_stop_media_play, &tmp_module_info);
	if (ret_int) {
		cp_app_log(error, vdu_demo, "cp_set_option_module() warning. module[%p].",
			tmp_channel.module);
		return -3;
	}
