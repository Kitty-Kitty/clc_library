...
//退出模块
cp_int32_t destroy_player_func_module(cp_player_func_manager_t *manager, cp_player_func_t *func);
//设置模块参数
...
cp_player_func_t* create_player_func_module(cp_player_func_manager_t *manager, const cp_player_func_info_t *info)
{
	return (cp_player_func_t*)cp_manager_create_module_by_register(manager, info);
}
//模块查询接口，表示从管理器中查询一个模块
cp_player_func_t* get_player_func_module(cp_player_func_manager_t *manager, const cp_player_func_info_t *info)
{
	...
	for (current = cp_queue_next(modules); current != modules;
		...
		func = (cp_player_func_t*)cp_queue_data(current, cp_func_t, queue);
...
//退出模块
cp_int32_t destroy_player_func_module(cp_player_func_manager_t *manager, cp_player_func_t *module)
{
	return 0;
}
//设置模块参数
