...
**************************************************************************/
#include "cp_hi3531_manager_info.h"
...
{
	cp_hi3531_manager_info_t			*tmp_info;
	...
	//分配info内存
	tmp_info = cp_palloc(manager->pool, cp_sizeof(cp_hi3531_manager_info_t));
	if (!tmp_info) {
		cp_manager_log(error, manager, "init_hi3531_manager() error. cp_palloc() alloc info error.");
		return CP_NULL;
	}
	else {
		cp_memset(tmp_info, 0, cp_sizeof(cp_hi3531_manager_info_t));
		*tmp_info = *info;
