	...
	cp_media_description_t *md)
{
	...
	if (!tmp_codec_info) {
		cp_module_log(error, ffmpeg_av_codec, "create_ffmpeg_av_audio_codec_info() error.");
		return CP_NULL;
	}
	...
	cp_audio_description_t *ad, AVCodecContext *avctx, AVFrame *frame)
{
	...
	if (!tmp_swr_ctx) {
		cp_module_log(error, ffmpeg_av_codec, "swr_alloc() failed!");
		return -1;
	}
	...
	if (!tmp_swr_ctx) {
		cp_module_log(error, ffmpeg_av_codec, "swr_alloc_set_opts() failed!");
		return -1;
	}
	...
	if (tmp_ret_int < 0) {
		...
		cp_module_log(error, ffmpeg_av_codec, "swr_init() failed! %s", ffmpeg_av_codec->error_text);
		return -2;
	}
	...
	if (ffmpeg_av_codec->audio_info->cp_convert_ad.samples != frame->nb_samples) {
		...
		if (tmp_ret_int < 0) {
			set_ffmpeg_av_codec_error_info(ffmpeg_av_codec, tmp_ret_int);
			cp_module_log(error, ffmpeg_av_codec, "swr_set_compensation() failed! %s", ffmpeg_av_codec->error_text);
			//return -2;
		}
	}
...
cp_int32_t unset_ffmpeg_av_audio_convert_context_info(cp_ffmpeg_av_codec_t *ffmpeg_av_codec)
{
	if (!ffmpeg_av_codec->audio_info ||
		!ffmpeg_av_codec->audio_info->audio_convert ||
	...
	AVCodecContext *avctx, AVFrame *frame, cp_media_packet_t *mp, cp_int32_t len, cp_int32_t flag)
{
	...
	if (ret_int) {
		cp_module_log(error, ffmpeg_av_codec, "set_ffmpeg_av_audio_convert_context_info() error.");
		return -1;
	}
	...
	if (ret_int) {
		cp_module_log(error, ffmpeg_av_codec, "create_ffmpeg_buf_info() error.");
		return -2;
	}
	...
	if (ret_int) {
		cp_module_log(error, ffmpeg_av_codec, "set_receive_media_description() error.");
		return -1;
	}
