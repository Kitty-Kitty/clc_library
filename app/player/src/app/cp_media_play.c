...
#include "cp_player_include.h"



cp_int32_t create_media_play(cp_player_t *player, cp_play_channel_info_t *cp_info)
{
	cp_int32_t							i = 0;
...
cp_int32_t destroy_media_play(cp_player_t *player, cp_play_channel_info_t *cp_info)
{
	return stop_media_play(player, cp_info);
}

cp_int32_t init_media_play(cp_player_t *player, cp_play_channel_info_t *cp_info)
{
	return 0;
}

cp_int32_t start_media_play(cp_player_t *player, cp_play_channel_info_t *cp_info)
{
	return 0;
}

cp_int32_t stop_media_play(cp_player_t *player, cp_play_channel_info_t *cp_info)
{
	cp_int32_t							i = 0;
...
cp_int32_t update_media_play(cp_player_t *player, cp_play_channel_info_t *cp_info)
{
	cp_int32_t							i = 0;
	cp_media_channel_info_t				*tmp_mci = CP_NULL;
	cp_module_t							*tmp_module = CP_NULL;
...
//创建一个音频处理模块
cp_module_t* create_audio_media_play(cp_player_t *player, cp_media_channel_info_t *mci)
{
	return create_av_media_play_module(player, mci, cp_player_manager_type_audio_func, cp_player_module_type_audio_func);
}

//创建一个视频处理模块
cp_module_t* create_video_media_play(cp_player_t *player, cp_media_channel_info_t *mci)
{
	return create_av_media_play_module(player, mci, cp_player_manager_type_video_func, cp_player_module_type_video_func);
}

//创建需要的音视频模块
cp_module_t* create_av_media_play_module(cp_player_t *player, cp_media_channel_info_t *mci,
...
******************************************************************************************************/
cp_module_t* update_audio_media_play(cp_player_t *player, cp_media_channel_info_t *mci)
{
	return update_av_media_play_module(player, mci, cp_player_manager_type_audio_func, cp_player_module_type_audio_func);
}
...
* 创建日期：2016-11-24 14:58:52
******************************************************************************************************/
cp_module_t* update_video_media_play(cp_player_t *player, cp_media_channel_info_t *mci)
{
	return update_av_media_play_module(player, mci, cp_player_manager_type_video_func, cp_player_module_type_video_func);
}
...
******************************************************************************************************/
cp_module_t* update_av_media_play_module(cp_player_t *player, cp_media_channel_info_t *mci, cp_int32_t manager_type, cp_int32_t module_type)
{
	cp_module_t									*tmp_module = CP_NULL;
	cp_module_manager_t							*tmp_manager = CP_NULL;
	cp_player_info_t							*tmp_player_info = (cp_player_info_t *)(player->app_info);
	...
	ret_int = cp_set_option_module(tmp_module,
		cp_player_func_set_option_update_media_info, &tmp_module_info);
	if (ret_int) {
...
******************************************************************************************************/
cp_int32_t stop_audio_media_play(cp_player_t *player, cp_media_channel_info_t *mci)
{
...
******************************************************************************************************/
cp_int32_t stop_all_audio_media_play(cp_player_t *player)
{
	cp_player_info_t							*tmp_player_info = (cp_player_info_t *)(player->app_info);
...
******************************************************************************************************/
cp_int32_t stop_video_media_play(cp_player_t *player, cp_media_channel_info_t *mci)
{
	return stop_av_media_play_module(player, mci);
}
...
******************************************************************************************************/
cp_int32_t stop_all_video_media_play(cp_player_t *player)
{
	cp_player_info_t							*tmp_player_info = (cp_player_info_t *)(player->app_info);
...
//停止一个播放
cp_int32_t stop_av_media_play_module(cp_player_t *player, cp_media_channel_info_t *mci)
{
	cp_module_info_t							tmp_module_info = { 0, };
	cp_channel_t								tmp_channel = { 0, };
	...
	ret_int = cp_set_option_module(tmp_channel.module,
		cp_player_func_set_option_stop_media_play, &tmp_module_info);
	if (ret_int) {
		cp_app_log(error, player,
...
******************************************************************************************************/
cp_int32_t stop_all_av_media_play_module(cp_player_t *player)
{
	cp_channel_t								*tmp_ret_channel = CP_NULL;
	cp_player_info_t							*tmp_player_info = (cp_player_info_t *)(player->app_info);
	cp_media_channel_info_t						*tmp_mci = CP_NULL;
	...
	while (tmp_ret_channel)	{
		...
		if (tmp_ret_channel->module) {
			...
			ret_int = cp_get_option_module(tmp_ret_channel->module,
				cp_player_func_get_option_media_channel_info, &tmp_mci);
			if (!ret_int && tmp_mci) {
