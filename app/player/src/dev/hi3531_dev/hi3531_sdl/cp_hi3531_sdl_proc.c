	...
	cp_video_frame_packet_t	*vfp)
{
	...
	for (tmp_current = cp_queue_next(tmp_queue); tmp_current != tmp_queue;
		...
		tmp_vfp.ms = surfact;
		tmp_vfp.mp = (cp_media_packet_t*)vfp;

		//将cp_video_frame_packet_t添加到链表中
		cp_queue_insert_tail(&(tmp_vfp.vfp_queue), &(vfp->queue));
