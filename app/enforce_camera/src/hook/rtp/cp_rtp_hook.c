...
cp_int32_t cp_open_queue_hook(cp_rtp_hook_t *hook, const cp_ecam_hook_info_t *ecam_hook_info, cp_queue_t *queue);
...
cp_rtp_hook_t* create_rtp_hook(cp_module_register_t *register_info, cp_socket_manager_t *manager, cp_rtp_hook_info_t *info)
{
	...
	cp_ecam_hook_register_t				*css_register_info = CP_NULL;
	...
	css_register_info = (cp_ecam_hook_register_t*)register_info;
...
cp_int32_t destroy_rtp_hook(cp_ecam_hook_manager_t *manager, cp_rtp_hook_t* rtp_hook)
{
	if (!manager || !rtp_hook) {
		return -1;
	}
...
cp_int32_t open_rtp_hook(cp_rtp_hook_t *rtp_hook, const cp_rtp_hook_info_t *rtp_hook_info)
{
	...
	cp_ecam_hook_stack_t			*tmp_stack;
	...
	//先执行入口函数
	ret_int = cp_open_queue_hook(rtp_hook, (const cp_ecam_hook_info_t *)rtp_hook_info, &rtp_hook->entry);
	if (ret_int) {
	...
	if (ret_int) {
		...
		ret_int = cp_open_queue_hook(rtp_hook, (const cp_ecam_hook_info_t *)rtp_hook_info, &rtp_hook->error);
		if (ret_int) {
		...
		ret_int = cp_open_queue_hook(rtp_hook, (const cp_ecam_hook_info_t *)rtp_hook_info, &rtp_hook->right);
		if (ret_int) {
	...
	if (tmp_sesssion && (tmp_sesssion->option & CP_SESSION_OPTION_SET_HISTORY)) {
		...
		tmp_stack = cp_palloc(tmp_sesssion->pool, cp_sizeof(cp_ecam_hook_stack_t));
		if (tmp_stack) {
			//初始化调用信息，并加入历史队列中
			CP_CSS_HOOK_STACK_INIT_FIELDS(tmp_stack);
			CP_CSS_HOOK_STACK_SET_VALUE(tmp_stack, (cp_ecam_hook_t *)rtp_hook, tmp_sesssion, tmp_sesssion->arg);
...
{
	cp_int32_t							ret_int = 0;
	cp_ecam_t							*tmp_ecam = (cp_ecam_t *)rtp_hook->manager->app;
	...
	switch (optname)
	{
	case cp_ecam_hook_set_option_start_heartbeat:
	{
	}
	break;
	case cp_ecam_hook_set_option_stop_heartbeat:
	{

	}
	break;
	case cp_ecam_hook_set_option_send_register:
	{
...
cp_int32_t cp_open_queue_hook(cp_rtp_hook_t *hook, const cp_ecam_hook_info_t *ecam_hook_info, cp_queue_t *queue)
{
	...
	for (current = cp_queue_next(queue); current != queue;
		...
		ret_int = cp_open_module(tmp_block->hook, (const cp_module_info_t *)ecam_hook_info);
		if (tmp_block->hook->current_hook) {
			ret_int = cp_open_module(tmp_block->hook->current_hook, (const cp_module_info_t *)ecam_hook_info);
		}
	}
